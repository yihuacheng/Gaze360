...
if __name__ == "__main__":
  ...
  if i in range(15): 
    ...
    if not os.path.exists(os.path.join(loadpath, f"evaluation-within/{tests}")):
      os.makedirs(os.path.join(loadpath, f"evaluation-within/{tests}"))

    print("Read data")
    dataset = dataloader.txtload(os.path.join(labelpath, tests), imagepath, 32, shuffle=False, num_workers=4, header=True)
    # dataset = dataloader.txtload(trainlabelpath, imagepath, 32, num_workers=4, header=True)
    ...
    for saveiter in range(begin, end+step, step):
      ...
      statedict = torch.load(os.path.join(savepath, f"Iter_{saveiter}_{modelname}.pt"), map_location={"cuda:1":"cuda:0"})
      ...
      count = 0
      with torch.no_grad():
        with open(os.path.join(loadpath, f"evaluation-within/{tests}/{saveiter}.log"), 'w') as outfile:
